	...
	$.post("/chooseMusicDirectory", {dir : current_dir}, function(responseJSON) {
		...
		songs.forEach(function(elem) {
		    ...
		    .done(function(responseJSONSong) {
		    	var song = $("<div class='song'><img src='../images/placeholder.png' style='float:left;width:48px;height:48px'><h5 class='song'>" + _title + " by " + _artist + "</h5></div>");

		    	if (typeof responseJSONSong.error == 'undefined') {
					var albumart = responseJSONSong.album.image[1]["#text"];
					
					if (typeof _title == 'undefined') {
						song = $("<div class='song'><img src='../images/placeholder.png' style='float:left;width:48px;height:48px'><h5 class='song'>Unknown by unknown </h5></div>");
					} else {
						song = $("<div class='song'><img src='../images/placeholder.png' style='float:left;width:48px;height:48px'><h5 class='song'>" + _title + " by " + _artist + "</h5></div>");
					}
					
					if (typeof albumart != "undefined") {
						song = $("<div class='song'><img src='" + albumart + "' style='float:left;width:48px;height:48px'><h5 class='song'>" + _title + " by " + _artist + "</h5></div>");
					}
				}
